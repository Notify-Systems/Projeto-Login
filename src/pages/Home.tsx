    ...
    return(
        <>
            <section className="flex flex-col justify-center items-center h-screen">
                ...
                <span className="text-[#888] text-[12px] mt-4 font-display italic md:text-[20px] md:mt-5">Seu tema atual é {theme}</span>
            </section>
            <section className="fixed bottom-5 left-3 flex flex-col gap-3">
                <button
                    type="button"
                    className={`w-[220px] h-[50px] rounded-lg border text-[25px] font-bold font-display transition duration:300
                ...
                <button
                    type="button"
                    onClick={() => navigate("/")}
                    className={`text-red-500 w-[220px] h-[50px] rounded-lg border border-[red] text-[25px] font-bold font-display 
                ...
                <button 
                    type="button" 
                    className={`w-[220px] h-[50px] rounded-lg border border-[red] text-[25px] font-bold font-display text-red-500 
                        ${actualTheme == "dark" ? "hover:text-[#FF5454] hover:border-[#FF5454]" : "text-black border-black hover:bg-[#EAEAEA]"}`}>
                    APAGAR CONTA
                </button>
            </section>
